/**\file PlanningUnitMADPDiscrete.h
 *
 * Authors:
 ...
 *
 * $Id: PlanningUnitMADPDiscrete.h 3611 2009-08-25 15:02:24Z mtjspaan $
 */
...
protected:
    ...
    std::map< Index, JointActionObservationHistoryTree*> 
...
public:
    ...
    Index IndividualToJointActionIndices(const Index* 
        ...
        {return (GetReferred()->IndividualToJointActionIndices(
                     indivActionIndices));}
    ...
    Index IndividualToJointObservationIndices(std::vector<Index>& inObs) const
        {return(GetReferred()->IndividualToJointObservationIndices(inObs));}
    /** \brief Returns a vector containing the indices of the
    ...
    std::vector<Index> JointToIndividualObservationIndices(Index joI) const
        {return(GetReferred()->JointToIndividualObservationIndices(joI));}
    ...
    JointToIndividualActionObservationHistoryIndicesRef( Index jaohI)
        const;    
    /**Returns a vector containing the indices of the indiv.
     ...
     * \li \a indivIs - the vector containing the individual indices.
     */
    Index IndividualToJointActionObservationHistoryIndex(Index t, 
            const std::vector<Index>& indivIs) const;
    ...
    JointActionObservationHistoryTree* 
    GetJointActionObservationHistoryTree(Index jaohI) const;
     ...
     */
    void GetJointActionObservationHistoryVectors (Index jaohI,
...
//            const JointBelief* p_jb = NULL, // the corresponding GIVEN jb of p
    ...
    double GetTransitionProbability(Index sI, Index jaI, Index sucSI) 
        const
        { return(GetReferred()->GetTransitionProbability(sI, jaI, sucSI));};
    /// Returns P(joI | jaI, sucSI ). Arguments are time-ordered.
    double GetObservationProbability(Index jaI, Index sucSI, Index joI) 
        const
        { return(GetReferred()->GetObservationProbability(jaI, sucSI, joI));};
    ...
    //functions for Interface_ProblemToPolicyDiscretePure
    ...
    const TransitionModelDiscrete* GetTransitionModelDiscretePtr() const
        { 
//            std::cerr << "GetTransitionModelDiscretePtr deprecated, use GetProblem->GetTGet()"<<std::endl;
            return(GetReferred()->GetTransitionModelDiscretePtr()); }
    
    const ObservationModelDiscrete* GetObservationModelDiscretePtr() const
        { return(GetReferred()->GetObservationModelDiscretePtr()); }
    
    //functions that implement the Interface_ProblemToPolicyDiscretePure
    ///
