/**\file PlanningUnitMADPDiscrete.cpp
 *
 * Authors:
 ...
 *
 * $Id: PlanningUnitMADPDiscrete.cpp 4022 2010-03-12 15:49:39Z mtjspaan $
 */
...
PlanningUnitMADPDiscrete(const PlanningUnitMADPDiscreteParameters &params,
    ...
    Referrer<MultiAgentDecisionProcessDiscreteInterface>(p)
{
...
void PlanningUnitMADPDiscrete::InitializeObservationHistories()
{
    ...
    else
    {
        ...
        {
            size_t nrOi = GetReferred()->GetNrObservations(i);
            oh_per_t.clear();
        ...
        while(johtQueue.size() > 0)
        {
            ...
            if(ts !=  thisLength ) //length 0 <-> ts 0 
            {
                ...
                _m_nrJointObservationHistoriesT.push_back(
                    JointObservationHistoryIndex - _m_firstJOHIforT[ts]);
                if(++ts !=  thisLength )
...
void PlanningUnitMADPDiscrete::InitializeActionHistories()
{
    ...
    else
    {
        ...
        for(Index i=0; i < GetNrAgents(); i++)
        {
            size_t nrAi = GetReferred()->GetNrActions(i);
            oh_per_t.clear();
...
ActionHistoryTree* PlanningUnitMADPDiscrete::CreateActionHistoryTree(Index agentI)
{
    ...
    //if(!GetReferred()->IsInitialActionEmpty())
...
void PlanningUnitMADPDiscrete::DeInitializeJointActionObservationHistories()
{
    ...
    // clear the map
#if 0 // TODO: fix this, segfaults currently, and hence leaks memory
    while(!_m_jointActionObservationHistoryTreeMap.empty())
    {
        delete _m_jointActionObservationHistoryTreeMap.begin()->second;
        _m_jointActionObservationHistoryTreeMap.erase(
            _m_jointActionObservationHistoryTreeMap.begin());
    }
#else // not sure whether everything will be deleted now
...
void PlanningUnitMADPDiscrete::InitializeActionObservationHistories()
{
    ...
    else
    {
        ...
        for(Index i=0; i < GetNrAgents(); i++)
        {
            size_t nrAOi = GetReferred()->GetNrActions(i) * 
                GetReferred()->GetNrObservations(i);
            oh_per_t.clear();
    ...
    CreateActionObservationHistoryTree(Index agentI)
{
    ...
    Index nr_AOHI_for_ts = aohI - _m_firstAOHIforT[agentI][ts];
    _m_nrActionObservationHistoriesT[agentI].push_back(nr_AOHI_for_ts);
...
void PlanningUnitMADPDiscrete::InitializeJointActionObservationHistories()
{
    if(_m_params.GetComputeJointActionObservationHistories())
    {
        ...
        const StateDistribution* sd = GetReferred()->GetISD();
        ...
        while(jaohtQueue.size() >= 1)
        {
            ...
            for(Index jaI = 0; jaI < nrJA; jaI++)
                for(Index joI = 0; joI < nrJO; joI++)
                {
                    ...
                    *new_jb = *jb;
                    double new_cond_p = new_jb->Update(*GetReferred(),jaI, joI);
        ...
        _m_nrJointActionObservationHistories = jaohI;
...
{
    MultiAgentDecisionProcessDiscreteInterface* p = GetReferred();
...
size_t PlanningUnitMADPDiscrete::GetNrObservationHistories(Index agentI) const
{
    ...
    //else
    throw("ERROR PlanningUnitMADPDiscrete::GetNrObservationHistories(Index agentI) index out of bounds or _m_nrObservationHistories not yet initialized?!\n");
} 
...
//TODO let functions below use ComputeHistoryArrays
void 
...
PlanningUnitMADPDiscrete::GetJointActionObservationHistoryVectors
    (Index jaohI, vector<Index> &jaIs, vector<Index> &joIs) const
{
    jaIs.clear();
    joIs.clear();
    //try if cached
...
PlanningUnitMADPDiscrete::GetJointActionObservationHistoryArrays
    (Index jaohI, Index t,  Index jaIs[], Index joIs[]) const
{
    //substract the offset to find out what is the index within the time-step-t-
    //observation-histories.
    Index jaohIwithinThisT = jaohI - _m_firstJAOHIforT.at(t);
   
    Index remainder = jaohIwithinThisT;
...
//    Index jao_array[vec_size];
    for(Index t2=0; t2 < vec_size ; t2++)
    {
        ...
        Index jaoI_t2 = remainder / b;
//        jao_array[t2] = jaoI_t2;
        remainder -= jaoI_t2 * b;

        jaIs[t2] = IndexTools::
            ActionObservation_to_ActionIndex(jaoI_t2, nrJA, nrJO);
        joIs[t2] = IndexTools::
            ActionObservation_to_ObservationIndex(jaoI_t2, nrJA, nrJO);
    }
    ...
    (Index johI, Index t,  Index joIs[]) const
{
    //substract the offset to find out what is the index within the time-step-t-
    //observation-histories.
    Index johIwithinThisT = johI - _m_firstJOHIforT.at(t);
    ...
    (Index agentI, Index aohI, Index t,  Index aIs[], Index oIs[]) const
{
    //substract the offset to find out what is the index within the time-step-t-
    //observation-histories.
    Index aohIwithinThisT = aohI - _m_firstAOHIforT.at(agentI).at(t);
    ...
    (Index agentI, Index ohI, Index t,  Index oIs[]) const
{
    //substract the offset to find out what is the index within the time-step-t-
    //observation-histories.
    Index ohIwithinThisT = ohI - _m_firstOHIforT.at(agentI).at(t);
    ...
    (Index agentI, Index ahI, Index t,  Index aIs[]) const
{
    //substract the offset to find out what is the index within the time-step-t-
    //action-histories.
    Index ahIwithinThisT = ahI - _m_firstAHIforT.at(agentI).at(t);
        ...
        Index t, const vector<Index>& observations) const
{
    return( ComputeHistoryIndex(
                t, 
                _m_firstOHIforT.at(agentI).at(t), 
                &observations[0],
        ...
        ) const
{
    ...
    size_t first_AOHI=_m_firstAOHIforT.at(agentI).at(t);
    size_t nrA = GetNrActions(agentI);
    size_t nrO = GetNrObservations(agentI);
    size_t nrAO = nrA * nrO;
        ...
        ) const
{
    ...
    size_t first_JAOHI=_m_firstJAOHIforT.at(t);
    size_t nrJA = GetNrJointActions();
    size_t nrJO = GetNrJointObservations();
        ...
        johI) const
{
    ...
    for (Index agentI=0; agentI<GetNrAgents(); agentI++)
        indivO_vec.push_back( vector<Index>::vector() );
...
{    
    ...
    {
        JointActionObservationHistoryTree* jaoht = 
            _m_jointActionObservationHistoryTreeVector[jaohI];
        ...
        ) const
{
    ...
    //action indices and observation indices. A bit further down we do the 
    //reverse (albeit for individual indices)
                                     ...
                                     PolicyGlobals::IndexDomainCategory cat) const
{
    ...
    case PolicyGlobals::TYPE_INDEX : 
        ...
        break;
    default:
        throw(E("JointToIndividualPolicyDomainIndices: incorrect JointPolicyDiscrete::PolicyGlobals::IndexDomainCategory"));
        break;
    }
}
    ...
    Index jdI, PolicyGlobals::IndexDomainCategory cat) const
{
    ...
    case PolicyGlobals::TYPE_INDEX : 
        ...
        break;
    default:
        throw(E("JointToIndividualPolicyDomainIndicesRef: incorrect JointPolicyDiscrete::PolicyGlobals::IndexDomainCategory"));
        break;
    }
}
...
//TODO - check if working, create copy for act-obs
Index 
        ...
        Index t, const vector<Index>& indivIs) const
{
    ...
    //joint observation sequences -> joint history index.
    Index jaohI = GetJointActionObservationHistoryIndex(t, jacs, jobs);
    return(jaohI);
...
size_t PlanningUnitMADPDiscrete::GetNrActionHistories(Index agentI) const
{
    ...
    //else
    throw("ERROR PlanningUnitMADPDiscrete::GetNrActionHistories(Index agentI) index out of bounds or _m_nrActionHistories not yet initialized?!\n");
} 
...
{
    return(GetReferred()->JointToIndividualActionIndices(joI));
}


JointBeliefInterface* 
PlanningUnitMADPDiscrete::GetJointBeliefInterface(Index jaohI) const
{
    // TODO \todo review the following:
    // this is an awkward construction. 
    // when beliefs are cached we return that and we typically don't want 
    // to touch this belief
    // when beliefs aren't cached we create a *new* belief, which we typically
    // want to delete when we're done using it...
    //
    // UPDATE: I'v modified this function so that it always returns a _new_
    ...
    if(_m_params.GetComputeJointBeliefs())
    {
        ...
        *jbi = *(_m_jBeliefCache.at(jaohI)); //make a copy
        return jbi;
    }
    else
    {
        ...
        for(unsigned i=0;i!=jaIs.size();++i)
            JB->Update(*GetReferred(),jaIs[i],joIs[i]);
#if DEBUG_PUDCGETJB
        if(!JB->SanityCheck())
        {
            JB->Print();
...
PlanningUnitMADPDiscrete::GetJAOHProbs(
    ...
    //input args:
    Index jaohI, // the jaohI for which we want to know the probs
    ...
    ) const
{
    if(p_jaohI == 0 && p_jb == NULL && jpol==NULL) //the cacheable case
        if(_m_params.GetComputeJointBeliefs() )
        {
            ...
            *jb = *_m_jBeliefCache[jaohI]; 
            return (_m_jaohProbs[jaohI]);
        }
    ...
    ) const
{
    ...
    Index jaI = jaIs[t_p];
    Index joI = joIs[t_p];
    double cond_P = jb->Update(*GetReferred(), jaI, joI);
//    Index next_p_jaohI = //???
...
//            jaohI,
    ...
    {
        Index jdI = 0;
        switch ( jpol->GetIndexDomainCategory() )
        {
            case OHIST_INDEX :
            ...
            case AOHIST_INDEX :
                ...
                break;
            default:
                throw E("PlanningUnitMADPDiscrete::GetJAOHProbsRecursively --- policy has unsupported IndexDomainCategory");
        }
...
{
    JointBelief dummy( GetNrStates() );
    double p =  GetJAOHProbs(&dummy, jaohI, p_jaohI, p_jb, jpol );
    return p;
    //\todo TODO, because this function does not need to return to joint belief,
    //the last belief update is unecessary. therefore we can still gain some
    //speed here...
...
{
    if(_m_params.GetComputeJointActionObservationHistories())
        return(_m_jaohConditionalProbs[jaohI]);
    //else
...
{
    if(_m_params.GetComputeJointObservationHistories())
        return(GetJointObservationHistoryTree(johI)->GetSuccessor(joI)->
                GetIndex());
    ...
    Index johI_wo = johI - _m_firstJOHIforT[t];
    ...
    return(johIsuc_wo + _m_firstJOHIforT[t+1]);
}
...
{
    if(_m_params.GetComputeIndividualObservationHistories())
        return(GetObservationHistoryTree(agI, ohI)->GetSuccessor(oI)->
                GetIndex());
...
{
    if(_m_params.GetComputeJointActionHistories())
        return(GetJointActionHistoryTree(jahI)->GetSuccessor(joI)->
                GetIndex());
    ...
    Index jahI_wo = jahI - _m_firstJAHIforT[t];
    ...
    return(jahIsuc_wo + _m_firstJAHIforT[t+1]);
}
...
{
    if(_m_params.GetComputeIndividualActionHistories())
        return(GetActionHistoryTree(agI, ahI)->GetSuccessor(oI)->
                GetIndex());
...
Index PlanningUnitMADPDiscrete::GetTimeStepForAOHI(Index agI,Index aohI) const
{
    ...
    if(_m_params.GetComputeIndividualActionObservationHistories())
        return(GetActionObservationHistoryTree(agI, aohI)->
        ...
        Index aohI, Index aI, Index oI) const
{
    ...
    else
    {
        ...
        Index aohI_wo = aohI - _m_firstAOHIforT.at(agI).at(t);
        Index aohIsuc_wo = aohI_wo * GetNrObservations(agI)*
            GetNrActions(agI) + aI * GetNrObservations(agI) +oI;
        return(aohIsuc_wo + _m_firstAOHIforT.at(agI).at(t+1));
    }
}
Index PlanningUnitMADPDiscrete::GetSuccessorJAOHI(Index jaohI, Index jaI,
                                                  Index joI) const
{
    Index jaohIsuc;
    // copied from GetSuccessorJOHI, look there for comments
    ...
    else
    {
        ...
        Index jaohI_wo = jaohI - _m_firstJAOHIforT[t];
        Index jaohIsuc_wo = jaohI_wo * GetNrJointObservations()*
...
PlanningUnitMADPDiscrete::GetJointActionObservationHistoryTree(Index jaohI)
    const
{
    if(_m_params.GetComputeJointActionObservationHistories())
        return(_m_jointActionObservationHistoryTreeVector.at(jaohI));
    else
    {
        ...
        map<Index,JointActionObservationHistoryTree*>::const_iterator cit
...
bool PlanningUnitMADPDiscrete::AreCachedJointToIndivIndices(
    PolicyGlobals::IndexDomainCategory pdc) const 
{
    switch ( pdc )
    {
    case PolicyGlobals::OHIST_INDEX :
    ...
    case PolicyGlobals::TYPE_INDEX :
        ...
        break;
    default:
        throw(E("Used a JointPolicyDiscrete::PolicyGlobals::IndexDomainCategory unknown to PlanningUnitMADPDiscrete!"));
        break;
    }				/* -----  end switch  ----- */
}

string PlanningUnitMADPDiscrete::SoftPrintPolicyDomainElement(
    Index agentI, Index dI, PolicyGlobals::IndexDomainCategory cat ) const
{
    switch ( cat )
    {
    case PolicyGlobals::OHIST_INDEX :
    ...
    case PolicyGlobals::TYPE_INDEX : 
        ...
        break;
    default:
        throw(E("SoftPrintPolicyDomain: incorrect JointPolicyDiscrete::PolicyGlobals::IndexDomainCategory"));
        break;
    }
    return("IMPOSSIBLE!");
}
...
size_t PlanningUnitMADPDiscrete::GetNrPolicyDomainElements(
    Index agentI, PolicyGlobals::IndexDomainCategory cat,
    size_t depth ) const
{
    switch ( cat )
    {
    case PolicyGlobals::OHIST_INDEX:
        if(depth==MAXHORIZON || depth==GetHorizon())
            ...
            return(_m_firstOHIforT.at(agentI).at(depth));
        break;
    case PolicyGlobals::AOHIST_INDEX:
        if(depth==MAXHORIZON || depth==GetHorizon())
            return GetNrActionObservationHistories(agentI);
        else
            return(_m_firstAOHIforT.at(agentI).at(depth));
        break;
    case PolicyGlobals::TYPE_INDEX : 
        throw(E(" GetNrPolicyDomainElements: types are not a valid policy domain for a PlanningUnitMADPDiscrete"));
        break;
    default:
        throw(E("incorrect JointPolicyDiscrete::PolicyGlobals::IndexDomainCategory"));
        break;
    }
}

PolicyGlobals::IndexDomainCategory PlanningUnitMADPDiscrete::GetDefaultIndexDomCat()
    const
{
